...
enum TabIdentifier: String, CaseIterable {
    case home, audio, compress, stitch, videogif, icns, aidoc, lark2pad, shutterCount, settings
}
    ...
    private func handleDroppedFiles(_ urls: [URL], shouldQuitAfter: Bool) {
        ...
        Task.detached(priority: .userInitiated) {
            ...
            if imageFiles.count > 1 {
                print("🧩 检测到多张图片，进入拼接模式 (isSilent: true)")
                await ImageStitcher.process(imageURLs: imageFiles, isSilent: true)
            } else {
