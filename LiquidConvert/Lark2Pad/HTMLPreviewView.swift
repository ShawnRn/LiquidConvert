...
struct HTMLPreviewView: NSViewRepresentable {
    ...
    func updateNSView(_ webView: WKWebView, context: Context) {
        ...
        let roundImages = (UserDefaults.standard.object(forKey: "lark2pad_round_images") == nil || UserDefaults.standard.bool(forKey: "lark2pad_round_images"))

        // Offload regex and heavy string processing to background queue
        Task.detached(priority: .userInitiated) {
            let styledHTML = Self.buildStyledHTML(from: currentHTML, isDark: isDark, roundImages: roundImages)
            await MainActor.run {
                // Ensure context hasn't changed while processing
    ...
    private static func buildStyledHTML(from html: String, isDark: Bool, roundImages: Bool) -> String {
        let extractedStyles = extractTagContent(tag: "style", from: html)
        let imgBorderRadius = roundImages ? "8px" : "0"
        let localCSS = """
        body {
    ...
    private static func lazyLoadedPreviewBody(_ body: String) -> String {
